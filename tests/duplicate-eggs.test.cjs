...
egg();c.hatchTest();assert.equal(c.owned().length,1);assert.equal(dragon.xp,30);assert.equal(dragon.level,2);
assert.equal(dragon.hunger,43);assert.equal(dragon.cleanliness,22);assert.equal(dragon.restUntil,now+1000);
assert(message.textContent.includes('+50 XP'));assert(message.textContent.includes('Niveau 2'));
c.hatchTest();assert.equal(dragon.xp,30,'double clic sans double récompense');
assert.equal(JSON.parse(data.get('owned'))[0].xp,30,'progression sauvegardée');
data.set('egg',JSON.stringify({dragonId:'fire',hatchAt:now+1000}));c.hatchTest();assert.equal(dragon.xp,30,'pas de récompense anticipée');
now+=1000;c.hatchTest();assert.equal(dragon.xp,80);
console.log('Œufs : nouveau dragon, doublon +50 XP, niveau, sauvegarde, soins conservés et double clic OK');
