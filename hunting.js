...
function startHuntingGame() {
    ...
    if (status) {
        status.textContent =
            "👀 Observe bien la zone... touche le canard dès qu'il apparaît !";
    }
...
function showHuntingDuck() {
    ...
    const maxX =
        Math.max(
            field.clientWidth - duckSize,
            20
        );

    const maxY =
        Math.max(
            field.clientHeight - duckSize,
            50
        );
    ...
    if (status) {
        status.textContent =
            "🦆 VITE ! Touche le canard !";
    }

    // Le canard reste visible seulement entre 0,8 et 1,3 seconde.
    const visibleTime =
        800 +
        Math.random() * 500;
        ...
        setTimeout(
            function() {
                if (
                    ...
                    if (status) {
                        status.textContent =
                            "💨 Trop tard ! Le canard s'est envolé...";
                    }
                    ...
                    if (message) {
                        message.textContent =
                            "Aucune viande récoltée cette fois.";
                    }
...
function catchHuntingDuck(event) {
    ...
    if (status) {
        status.textContent =
            "🎯 Bien joué ! Canard attrapé !";
    }

    if (message) {
        message.textContent =
            "🍖 +1 viande ajoutée dans ton sac !";
    }
